...
# All directions are inverted in y-direction...
def direction(c):
    return {ord("^"): (0,-1), ord("v"): (0,1), ord("<"): (-1,0), ord(">"): (1,0)}[c]
def rotate(d, left):
    return (-d[1], d[0]) if left else (d[1], -d[0])

def next_straight(p, d, field):

    s = ""

    # We know, that we have to go left!
    if field[add(p, rotate(d, True))] == ord("#"):
        s = "R"
        d = rotate(d, True)
    elif field[add(p, rotate(d, False))] == ord("#"):
        s = "L"
        d = rotate(d, False)
    else:
        # We're done!
    ...
    i = 0
    while field[add(p, d)] == ord("#"):
        i += 1
        p = add(p, d)

    s += str(i)

    # are we done?, movement string, new position, current direction
    return False, s, p, d

def create_movement_command(field, pos):

    # move until a pos is not in field, then turn.
    movement = ""
    done, s, p, d = next_straight(pos, direction(field[pos]), field)
    while not done:
        movement += s
        done, s, p, d = next_straight(p, d, field)

    return movement

def split(l):
    return re.findall("[R|L][\d]+", l)

def find_all_combinations(l):
    l = split(l)

    for n in range(min(len(l)+1, 10)):
        for i in range(len(l)):
            yield "".join(l[i:i+n])

def try_combination(l, sub_lists):

    three_tuples = combinations(sub_lists, 3)
    for t in three_tuples:
        # To avoid accidentally replace over boundaries where ABC was before.
        s = l.replace(t[0], "_").replace(t[1], "_").replace(t[2], "_")

        # All sub commands must be short enough
        if len(split(t[0])) <= 5 and len(split(t[1])) <= 5 and len(split(t[2])) <= 5:
            # And the final one as well without remainders.
            if len(s) <= 10  and s == "_" * len(s):
                return True, l.replace(t[0], "A").replace(t[1], "B").replace(t[2], "C"), t
    return False, "", None
...
if __name__ == "__main__":
    ...
    found, bot_command, (a, b, c) = try_combination(command, list(set(find_all_combinations(command))))
    ...
    if found:
        ...
        machine.set_inputs(list(map(ord, ",".join(list(bot_command)))) + [ord("\n")])
        #print("".join(map(chr, machine.get_outputs())))
        split_a = "".join(list(map(lambda x: x[0]+","+x[1:]+",", split(a))))[:-1]
        machine.set_inputs(list(map(ord, split_a)) + [ord("\n")])
        #print("".join(map(chr, machine.get_outputs())))
        split_b = "".join(list(map(lambda x: x[0]+","+x[1:]+",", split(b))))[:-1]
        machine.set_inputs(list(map(ord, split_b)) + [ord("\n")])
        #print("".join(map(chr, machine.get_outputs())))
        split_c = "".join(list(map(lambda x: x[0]+","+x[1:]+",", split(c))))[:-1]
        machine.set_inputs(list(map(ord, split_c)) + [ord("\n")])
        #print("".join(map(chr, machine.get_outputs())))
        machine.set_inputs([ord("n"), 10])

        # solution for puzzle 2:
        print(machine.get_outputs()[-1])

# solution for 15.01: 4688
# solution for 15.02: 714866
