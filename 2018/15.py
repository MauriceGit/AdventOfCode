...
def closest_target(orig_graph, units, current_index):
    ...
    all_paths.sort(key=lambda x: (len(x), x[0][0], x[0][1]))
...
def get_inrange_target(p, units, team):
    ...
    for i, u in enumerate(units):
        if u[2] != team and length(sub(u[0], p)) == 1 and (t_index == -1 or u[1] < units[t_index][1]):
            t_index = i
    return t_index
...
def main():
    ...
    print(units)
    print(rounds)
    print((rounds-1) * sum(x[1] for x in units))
