...
def wrap2(grid, pos, d):
    ...
    if section == 1 and d[0] == 1:
        return (4*block_size-1, block_size-pos[1] + 2*block_size), (-1,0)
    if section == 1 and d[1] == -1:
        return (block_size-(pos[0]-2*block_size), block_size), (0,1)

    if section == 2 and d[0] == -1:
        return (block_size-(pos[1]-block_size) + 3*block_size, 3*block_size-1), (0,-1)
    if section == 2 and d[1] == -1:
        return (block_size-pos[0] + 2*block_size, 0), (0,1)
    if section == 2 and d[1] == 1:
        return (block_size-pos[0] + 2*block_size, 3*block_size-1), (0,-1)

    if section == 3 and d[1] == -1:
        return (2*block_size, pos[0]-block_size), (1,0)
    if section == 3 and d[1] == 1:
        return (2*block_size, block_size-(pos[0]-block_size) + 2*block_size), (1,0)

    if section == 4 and d[0] == 1:
        return ((block_size-1)-(pos[1]-block_size) + 3*block_size, 2*block_size), (0,1)

    if section == 5 and d[0] == -1:
        return (block-(pos[1]-2*block_size)+block_size, 2*block_size-1), (0,-1)
    if section == 5 and d[1] == 1:
        return (block_size-(pos[0]-2*block_size), 2*block_size-1), (0,-1)

    if section == 6 and d[0] == 1:
        return (3*block_size-1, block_size-(pos[1]-2*block_size)), (-1,0)
    if section == 6 and d[1] == -1:
        return (3*block_size-1, block_size-(pos[0]-3*block_size) + block_size), (-1,0)
    if section == 6 and d[1] == 1:
        return (0, block_size-(pos[0]-3*block_size) + block_size), (1,0)
