...
def combinations(l):
    for n in range(2, min(len(l)+1, 20), 2):

        for i in range(0, len(l), 2):
            add = 0
            add_s = 0
            while i+add_s < len(l) and l[i+add_s] not in "RLABC-":
                add_s += 1

            # for a 2-digit number, we need another char
            while i+n+add < len(l) and l[i+n+add] not in "RLABC-":
                add += 1
...
def find_sublists(l, c=1, sublists=[]):
...
if __name__ == "__main__":
    ...
    #print(list(combinations(command))[:20])
    #print('R8R8R4R4R8L6L2R4R4R8R8R8L6L2')
    print(command)

    print(list(combinations(command)))

    sub_lists = find_sublists(command)
    print(sub_lists)
