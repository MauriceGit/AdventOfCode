...
# steps throught the path. Returns the current new position of the bot.
def go_to(machine, path):

    pos = (0,0)
    for c in path:
        machine.set_input(c)
        last_out = machine.get_outputs()[0]
        if last_out != 1:
            print("Something went wrong. Unexpected wall!")
            return pos
        pos = add(pos, c)
    return pos

def try_step(machine, c):
    machine.set_input(c)
    out = machine.get_outputs()[0]
    if out != 0:
        machine.set_input(reverse(c))
        # output needs to be queried after the last input!!!! Even if we don't care about it!
        machine.get_outputs()
    return out

# True/False if oxygen found. And the new path to the possibly new position.
def step(machine, path, pos, visited, field, c):

    if not visited[add(pos,c)]:
        out = try_step(machine, c)
        ...
        visited[add(pos,c)] = True

        return out == 2, [path+[c]]
    return False, []

# returns a list of new directions. Equal to [path+step, ...]
def calc_new_paths(path, visited, field):

    machine = IntCode(g_data)
    # Should never go wrong.
    pos = go_to(machine, path)

    # if possible, try all neighbor locations.
    new_steps = []

    found1, p = step(machine, path, pos, visited, field, 1)
    new_steps.extend(p)
    found2, p = step(machine, path, pos, visited, field, 2)
    new_steps.extend(p)
    found3, p = step(machine, path, pos, visited, field, 3)
    new_steps.extend(p)
    found4, p = step(machine, path, pos, visited, field, 4)
    new_steps.extend(p)
    ...
    return new_steps, found1 or found2 or found3 or found4, loc


def fill_oxygen(pos, field):

    if len(field) == 0 or pos not in field:
        return 0

    del field[pos]

    i1 = 1+fill_oxygen(add(pos, 1), field)
    i2 = 1+fill_oxygen(add(pos, 2), field)
    i3 = 1+fill_oxygen(add(pos, 3), field)
    i4 = 1+fill_oxygen(add(pos, 4), field)

    return max(i1, i2, i3, i4)
...
if __name__ == "__main__":
    ...
    frontier, found, _ = calc_new_paths([], already_visited, field)
    ...
    while True:
        ...
        for path in frontier:
            # From position at path, get all new possible positions reachable.
            f, found, loc = calc_new_paths(path, already_visited, field)
            new_frontier.extend(f)
            if found:
                # solution for puzzle 1
                print(min(map(len, new_frontier)))
    ...
    print(fill_oxygen(oxygen_pos, field))
